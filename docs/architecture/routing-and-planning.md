...
Shortcut matching is intentionally narrow. Rules are explicit Python checks, not a broad natural
...
roots are rejected by the helper or allowed to fall through to the LLM planner path.
...
The shortcut layer does not add network, write, dangerous, Git mutation, process mutation, archive
